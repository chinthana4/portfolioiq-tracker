...
export default function DashboardPage() {
  ...
  useAutoRefresh(fetchSummary, REFRESH_MS);
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        ...
        {lastUpdated && (
          <div style={{ fontSize: 11, color: 'var(--text2)', textAlign: 'right' }}>
            ...
            Auto-refresh every 60s · Last: {lastUpdated.toLocaleTimeString('en-GB')}
          </div>
        )}
      </div>
      ...
      {!summary || summary.transaction_count === 0 ? (
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16 }}>
            <KPICard label="Total Invested"  value={summary.total_invested} />
            <KPICard label="Current Value"   value={summary.total_value} />
            <KPICard label="Total P&L"       value={summary.total_pnl}      type={pnlType} />
            ...
            <div className="card">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={summary.by_platform}>
                  ...
                  <YAxis tick={{ fill: 'var(--text2)', fontSize: 11 }} tickFormatter={v => `£${Math.abs(v) >= 1000 ? (v/1000).toFixed(0)+'k' : v}`} />
                  <Tooltip formatter={v => fmt(v)} contentStyle={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 8 }} />
          ...
          <div className="card">
            <div style={{ fontWeight: 600, marginBottom: 16 }}>Holdings Summary</div>
            <div style={{ overflowX: 'auto' }}>
              <table>
                <thead>
                  <tr><th>Ticker</th><th>Name</th><th>Units</th><th>Invested</th><th>Value</th><th>P&L</th><th>ROI</th><th>Risk</th></tr>
                </thead>
                <tbody>
                  {summary.by_share.sort((a,b) => b.current_value - a.current_value).map(s => (
                    <tr key={s.ticker}>
                      ...
                      <td>{s.share_name}</td>
                      <td>{Number(s.units).toFixed(4)}</td>
                      <td>{fmt(s.cost_basis)}</td>
                      <td>{fmt(s.current_value)}</td>
                      <td className={s.pnl >= 0 ? 'positive' : 'negative'}>{fmt(s.pnl)}</td>
